...
\noindent\textbf{定理 17.1.4}\quad 设函数 $f(x,y)$ 及其偏导数 $f'_x(x,y)$ 在区域 $D=[a,b]\times[c,d]$ 上连续, 且 $\varphi(x)\ (x\in[a,b])$ 是满足 $c\leqslant \varphi(x)\leqslant d$ 的可微函数, 则函数
\[
...
\noindent\textbf{证明}\quad 对任意给定的 $x_0\in[a,b]$, 由 $c\leqslant\varphi(x_0)\leqslant d$ 及 $f(x,y)$ 在 $D$ 的连续性知, $f(x_0,y)$ 在 $[c,\varphi(x_0)]$ 上连续, 所以 $I(x_0)=\int_c^{\varphi(x_0)}f(x,y)\mathrm dy$ 存在. 因此 $I(x)$ 在 $[a,b]$ 上有定义。
...
\noindent\textbf{定理 17.2.2（魏尔斯特拉斯定理）}\quad 设函数 $f(x,y)$ 在 $E\times[c,+\infty)$ 上有定义, 其中 $E\subset\mathbb R$ 是一个区间. 若存在函数 $g(y)(y\in[c,+\infty))$, 使得对于 $\forall x\in E$ 及 $\forall y\in[c,+\infty)$, 有 $|f(x,y)|\leqslant g(y)$, 并且 $\int_c^{+\infty}g(y)\mathrm dy$ 收敛, 则 $\int_c^{+\infty}f(x,y)\mathrm dy$ 在 $E$ 上绝对一致收敛。
...
\noindent\textbf{证明}\quad 充分性. 若无穷积分 $\int_0^{+\infty}f(x)\mathrm dx$ 收敛, 则它对参数 $\alpha\in(0,+\infty)$ 一致收敛. 又 $e^{-\alpha x}$ 关于 $x$ 单调, 且对 $\alpha\in(0,+\infty)$ 有 $|e^{-\alpha x}|\leqslant1$, 由阿贝尔判别法知 $\int_0^{+\infty}e^{-\alpha x}f(x)\mathrm dx$ 在 $(0,+\infty)$ 上一致收敛。
...
\noindent\textbf{例 17.3.2}\quad 求 $\mathbb R^n$ 中单位球体 $D:x_1^2+x_2^2+\cdots+x_n^2\leqslant1$ 的体积。
