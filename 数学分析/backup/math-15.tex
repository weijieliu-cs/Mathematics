...
\noindent\textbf{性质 15.2.8}\quad 设函数 $f(x),g(x)$ 在 $D$ 上可积, 且对于 $\forall x\in D$, 有 $f(x)\le g(x)$, 则
\[
...
\noindent\textbf{定理 15.3.2}\quad 设 $D=\{(x,y):a\le x\le b,\varphi_1(x)\le y\le\varphi_2(x)\}$ 是 X 型区域, 函数 $f(x,y)$ 在 $D$ 上可积, 且对于 $\forall x\in[a,b]$,
\[
...
\noindent\textbf{例 15.4.1}\quad 计算闭单位圆盘 $\Omega=\{(x,y):x^2+y^2\le1\}$ 的面积。
...
\noindent\textbf{例 15.5.1}\quad 设 $D=\{(x,y):0\le x\le1,y\in\mathbb R\}$, 则容易看出, 对于 $\forall R>0$ 及 $\forall R_2>R_1>1$, $D\cap D_R$ 是一个闭区域, 而 $D\cap D(R_1,R_2)$ 是两个闭区域。
...
\noindent\textbf{定理 15.5.2}\quad 设 $D\subset\mathbb R^2$ 是无界闭区域, 函数 $f(x,y)$ 在 $D$ 上有定义, 在 $D$ 的任何可求面积的有界闭子区域上可积, 并且对于 $\forall(x,y)\in D$, 有 $f(x,y)\ge0$, 则无穷重积分
\[
  \iint_D f(x,y)\mathrm dx\mathrm dy
\]
...
\noindent\textbf{注}\quad 当 $1<\alpha\le2$ 时,
\[
...
\]
由例 15.5.3 知, 当 $2-\dfrac\alpha2>1$, 即 $\alpha<2$ 时, 原瑕重积分收敛; 当 $2-\dfrac\alpha2\le1$, 即 $\alpha\ge2$ 时, 原瑕重积分发散。
...
\noindent\textbf{注 2}\quad 由解法 2, 我们容易将有界闭区域上具有一个瑕点的无界函数的重积分与无界区域上的重积分进行转化. 例如, 从例 15.5.3 我们可以容易地构造以原点为心的单位圆盘 $D$ 内的一个闭区域 $\Omega$, 使得 $0\in\partial\Omega$ 且当 $2\le\alpha<3$ 时, 无穷重积分
\[
