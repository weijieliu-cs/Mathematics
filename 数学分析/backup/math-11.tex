...
\noindent\textbf{解}\quad 由几何级数的性质知, 当 $|x|<1$ 时, 该幂级数是收敛的. 当 $|x|\ge1$ 时, 由
\[
  \lim_{n\to\infty}x^n\ne0,
\]
...
\]
则由定理 11.1.1 有 $R\ge |x_0|$, 但 $R\le |x_1|$. 这是因为倘若 $R>|x_1|$, 则存在 $|x_2|>|x_1|$, 使得 $\sum_{n=0}^{+\infty}a_nx_2^n$ 收敛. 由定理 11.1.1 知 $\sum_{n=0}^{+\infty}a_nx_1^n$ 也必收敛, 这与假设矛盾。
...
\[
  ...
  \quad(n=0,1,2,\cdots).
\]
由于函数序列 $\{u_n(x)\}$ 与 $x$ 无关且 $\sum_{n=0}^{+\infty}u_n(x)$ 收敛, 因此它关于 $x\in[-R,0]$ 是一致收敛. 而 $v_n(x)$ 关于固定的 $x\in[-R,0]$ 是 $n$ 的单调下降函数, 且对所有 $x\in[-R,0]$ 及所有 $n\in\mathbb N$, 有 $|v_n(x)|\le1$. 因此由函数项级数的阿贝尔判别法知在 $[-R,0]$ 上一致收敛. 证毕。
...
当 $\alpha\le-1$ 时, (11.3.2) 式在 $(-1,1)$ 内成立;
