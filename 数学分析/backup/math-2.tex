...
\noindent\textbf{定义 2.2.1}\quad 设 $\{x_n\}$ 是一个序列. 若 $\exists M>0$, 对 $\forall n$, 有 $|x_n|\le M$ 成立, 则称 $\{x_n\}$ 是\textbf{有界的}。
...
\textbf{充分性}\quad 设 $\{x_n\}$ 是一个柯西序列, 对 $\varepsilon_0=1$, 则 $\exists N$, 当 $n\ge N+1>N$ 时, 有
\[
  |x_n-x_{N+1}|<1,
\]
...
\noindent\textbf{例 2.5.3}\quad 给定序列 $\{x_n\}$ 及常数 $\alpha\ge2$, 令
\[
